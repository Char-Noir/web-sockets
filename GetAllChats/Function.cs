...
public class Function
{
    ...
    public async Task<APIGatewayProxyResponse> FunctionHandler(APIGatewayProxyRequest request, ILambdaContext context)
    {
        ...
        if(chats.Any(chat => chat.ChatId == lastId)){
            chats = chats.SkipWhile(chat => chat.ChatId != lastId).Take(pageSize).ToList();
        }
        else{
        ...
        var result = chats.Select(async chat=>new GetAllChatsResponseItem(chat){
            User1 = await GetUser(chat.user1),
            User2 = await GetUser(chat.user2)
        });
